...
class LoginSerializer(serializers.Serializer):
    ...
    token = serializers.CharField(max_length=255, read_only=True)
    userType = serializers.CharField(max_length=10, read_only=True)
    ...
    def validate(self, data):
        ...
        return {'email': user.email, 'token': user.token, 'userType': user.userType, 'id': user.id}
...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ('id', 'email', 'password', 'userType', 'patientProfile')
